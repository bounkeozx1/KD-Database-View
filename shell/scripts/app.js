...
/**
 * The Facebook badge, on the photograph itself.
 *
 * The screenshot comes out of the PPTX as an ordinary document, so it is
 * reachable through the Documents tab like anything else — but only if you
 * think to look there. Recruiters check the face and the Facebook page in one
 * motion, so the way in belongs where they are already looking.
 *
 * Anchored to the photo rather than placed in the layout beside it: the detail
 * page reflows from two columns to one below 900px, and a control that lives
 * in the layout moves with it. This one is positioned inside .vbp-photo, so it
 * sits on the same corner of the same picture at every width.
 */
function _fbBadge(w) {
  const docs = (w && w.documents && w.documents.facebook) || [];
  if (!docs.length) return '';
  const label = bi('ເບິ່ງໜ້າ Facebook', 'View Facebook page', 'ดูหน้า Facebook', 'Facebook 페이지 보기');
  return '<button class="vbp-fb" title="' + esc(label) + '" aria-label="' + esc(label) + '"' +
    ' onclick="event.stopPropagation();openDocView(\'' + esc(w.uid) + '\',\'facebook\',0)">' +
    '<svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">' +
    '<path d="M22 12.06C22 6.5 17.52 2 12 2S2 6.5 2 12.06c0 5 3.66 9.15 8.44 9.94v-7.03H7.9v-2.9h2.54V9.85c0-2.5 1.49-3.89 3.77-3.89 1.09 0 2.24.2 2.24.2v2.46h-1.26c-1.24 0-1.63.77-1.63 1.56v1.88h2.78l-.45 2.9h-2.33V22c4.78-.79 8.44-4.93 8.44-9.94z"/>' +
    '</svg><span class="vbp-fb-lbl">Facebook</span></button>';
}
...
function _renderDetailBody(w, g) {
  ...
  const leftCol =
    '<div class="bento-card vbp-card' + editCls + '"' + photoAttr + '>' +
      '<div class="vbp-photo">' + photoImg + editHint + _fbBadge(w) + '</div>' +
...
['view-overlay','form-overlay','group-overlay','confirm-overlay','settings-overlay','import-overlay','scan-overlay','docview-overlay','photo-editor-overlay','export-overlay','create-overlay','customize-overlay','trash-overlay','importreport-overlay','imgreview-overlay'].forEach(id => {
